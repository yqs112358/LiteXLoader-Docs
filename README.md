...
1. **跨语言**插件开发支持   
    借助`ScriptX`强大的抽象能力，`LiteXLoader`支持使用**多种**不同的脚本语言编写插件，同时保证面向各种语言的接口统一
...
2. **功能强大，设施完备**  
    1. 提供众多的游戏API支持，为插件开发打好基础
    2. 配备大量的基础设施，如统一配置文件接口、统一日志接口、文件系统、数据库等等  
       从根源上解决插件开发的众多痛点
3. **兼容性强**  
     1. LXL支持通过Wine在Linux平台运行，给Linux平台也带来自由的插件体验  
        一次编写，多平台共享
     2. 兼容绝大多数其他主流插件加载器（`CSR` `PYR`等）
     3. 后续版本更新时将保证API**向后兼容**，避免出现插件需要一再重写的情况。脚本插件不需要随版本更新而修改代码，一次编写，全版本适配。
4. **模块化，高性能**  
