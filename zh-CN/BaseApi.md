...
- 参数：
  ...
  - callback : `Function`  
    ...
    其中`args`为按空格为分界分割得到的命令字符串数组，类型为`Array<String,String...>`  
- 返回值：是否成功注册
- 返回值类型：`Boolean`
...
说明：命令回调函数解决了插件注册多个命令时，在命令监听事件函数处代码过于混乱的问题。  
设置了回调函数之后，在你注册的这个命令被执行的时候，回调函数就会被调用，同时LXL会自动帮你把命令参数分割成数组，方便处理。  

以Js语言为例：注册`mc.registerCmd("land buy", "购买领地", 0, function(args){ .... } );` 之后，  
当你执行命令`land buy abcde 12345`的时候，这个回调函数就会被调用  
回调函数参数args被传入一个数组：[ "land buy", "abcde" , "12345" ]  
这时，相关的处理就变得异常方便。注意，`args[0]`为你注册的这个命令本身

注：在调用回调函数之后，传统的的命令执行事件仍然会被激发一次。  
如果你不喜欢回调函数的这种形式，可以继续使用传统的命令监听方式。
